...
func Test_prompt(t *testing.T) {
	...
	// read the output of out prompt() func from out read pipe
	out, _ := io.ReadAll(r)

	// perform our test
	if string(out) != "-> " {
		t.Errorf("incorrect promptL expected -> but got %s", out)
	}
}
...
func Test_intro(t *testing.T) {
	...
	if !strings.Contains(string(out), "Enter a whole number") {
		t.Errorf("intro text not correct; got %s", out)
	}
}
...
func Test_checkNumbers(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "decimal", input: "2.0", expected: "Please enter a whole number!"},
		{name: "quit", input: "q", expected: ""},
	}

	for _, e := range tests {
		input := strings.NewReader(e.input)
		res, _ := checkNumbers(bufio.NewScanner(input))
